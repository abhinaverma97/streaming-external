    ...
    return (
        <div ref={ref} className={`relative flex flex-col w-full ${className}`}>
            <div
                ...
                className="flex items-center justify-between bg-white/[0.02] hover:bg-white/[0.05] border border-white/[0.05] hover:border-white/10 rounded-xl px-3 py-2.5 text-xs text-white/80 cursor-pointer transition-all duration-200"
            >
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-[#0b0c10] border border-white/10 rounded-xl overflow-hidden z-[100] shadow-2xl max-h-48 overflow-y-auto no-scrollbar">
                    {options.map((opt) => (
                        <div
                            ...
                            className={`px-3 py-2.5 text-xs cursor-pointer transition-colors ${value === opt.value ? 'bg-white/10 text-white' : 'text-white/60 hover:bg-white/5 hover:text-white/90'}`}
                        >
                            {opt.label}
                        </div>
                    ))}
